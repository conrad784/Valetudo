    ...
    pollMap() {
        ...
        if (now.getTime() - 600 > this.lastMapPoll.getTime()) {
            ...
            this.sendCloud({"method": "get_map_v1"}).then(res => {
                Logger.info("started sendCloud function");
                if (Array.isArray(res) && res.length === 1) {
                    Logger.info("sendCloud array:", JSON.stringify(res));
                    ...
                    if (StatusStateAttribute && StatusStateAttribute.isActiveState) {
                        Logger.info("has an active state:", JSON.stringify(StatusStateAttribute));
                        repollSeconds = 2;
                    }
                    ...
                    Logger.info("Repoll Seconds", JSON.stringify(repollSeconds));
